...
if ($_POST) {
  if (!empty($_POST['username']) && !empty($_POST['password'])) {
    $readFile = getUserPassPair();
    list($valid, $errCode) = checkCredentials($username, $password, $readFile);
...
{
  $userFile = "users.txt";
  $userFileLink = fopen("users.txt", "r");
  $userFileContents = fread($userFileLink, filesize($userFile));
  $userPassPair = explode("||>><<||", $userFileContents);
  fclose($userFileLink);
  foreach ($userPassPair as $value) {
    list($username, $pass) = explode(',', $value);
    $item = array(
      'username' => $username,
      'password' => $pass
    );
...
<head>
  <title>Txt File Assignment</title>
...
<body>
  <div id="header">
    <h1>Super Insecure Login Form</h1>
  </div>
  <div class="container">
